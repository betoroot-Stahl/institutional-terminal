# =========================================================
# INSTITUTIONAL TERMINAL PRO 6.0
# =========================================================
...
for ticker, peso in pesos.items():
    ...
    direcao = info["var_day"]

    micro_fluxo = info["var_5m"]
...
score_win += score_etf_win

score_wdo += score_etf_wdo
...
impacto_dxy = dxy_var * 40

score_win -= impacto_dxy

score_wdo += impacto_dxy
...
with s1:
    ...
    if "Compra Forte" in status_win:

        st.success(f"WIN → {status_win}")

    elif "Venda Forte" in status_win:

        st.error(f"WIN → {status_win}")

    else:

        st.warning(f"WIN → {status_win}")

with s2:

    if "Compra Forte" in status_wdo:

        st.success(f"WDO → {status_wdo}")

    elif "Venda Forte" in status_wdo:

        st.error(f"WDO → {status_wdo}")
